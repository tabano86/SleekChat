...
local UI = Util.singleton("UI", function()
    ...
    function self.Initialize(instance)
        ...
        instance.InputBox:SetScript("OnEnterPressed", function(box)
            ...
            if text and text ~= "" then
                self.SendMessage(instance, text)
                box:SetText("")
            end
        end)
    ...
    function self.CreateSidebar(instance)
        ...
        for i, tabName in ipairs(instance.db.profile.tabs) do
            ...
            btn:SetText(tabName)
            btn:SetScript("OnClick", function() self.SwitchTab(instance, tabName) end)
            instance.Sidebar.buttons[tabName] = btn
        end
    end

    function self.SwitchTab(instance, tabName)
    ...
    function self.RefreshMessages(instance)
        ...
        local messages = SleekChat.History.GetMessages(instance, instance.currentTab) or {}
        local yOffset = 0
        for i, msg in ipairs(messages) do
            local frame = self.CreateMessageFrame(instance, msg, yOffset)
    ...
    function self.CreateMessageFrame(instance, msg, yOffset)
    ...
    function self.RefreshPinned(instance)
    ...
    function self.SendMessage(instance, text)
    ...
    function self.UpdateBackground(instance)
