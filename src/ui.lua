...
function UI.Initialize(instance)
    ...
    -- Pinned messages area at the top of the ChatArea
    ...
    -- Scroll frame for chat messages below the pinned area
...
-- Create vertical sidebar buttons for each chat group.
...
-- Rebuilds the content area based on the current tab’s messages.
function UI.RefreshMessages(instance)
    ...
    local messages = SleekChat.History.GetMessages(instance, instance.currentTab)
    local yOffset = 0
...
-- Creates a frame for an individual chat message with a pin button.
...
-- Refresh the pinned messages area.
...
-- When a new message is added, refresh if it belongs to the current tab.
