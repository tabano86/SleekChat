...
AceHook.registry = AceHook.registry or setmetatable({}, { __index = function(tbl, key)
    tbl[key] = {}
    return tbl[key]
end })
...
function hook(self, obj, method, handler, script, secure, raw, forceSecure, usage)
    ...
    if uid then
        ...
        if handlers[uid] == handler then
            -- turn on a decative hook, note enclosures break this ability, small memory leak
            actives[uid] = true
            return
        elseif obj then
...
function AceHook:Unhook(obj, method)
    ...
    if obj then
        ...
        if scripts[uid] and obj:GetScript(method) == uid then
            ...
            scripts[uid] = nil
        elseif obj and self.hooks[obj] and self.hooks[obj][method] and obj[method] == uid then
            -- unhooks methods
        ...
        if self.hooks[method] and _G[method] == uid then
            -- unhooks functions
